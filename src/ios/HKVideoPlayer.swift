...
@objc(HKVideoPlayer) class HKVideoPlayer: CDVPlugin {
    ...
    @objc(playPlaylist:)
    func playPlaylist(command:CDVInvokedUrlCommand){
        guard let playlist = command.argument(at: 0) as? Array<Any> else {
            return
        }
        
        var playlistItem: [AVPlayerItem] = []
        for item in playlist {
            playlistItem.append(AVPlayerItem(url: URL(string: item as! String)!))
        }
        presentPlayList(queue: playlistItem)
    }
    ...
    private func presentPlayList(queue: [AVPlayerItem]) {
        let playlistItem = AVQueuePlayer(items: queue)
        self.playerViewController.player = playlistItem
